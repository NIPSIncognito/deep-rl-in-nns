...
class BaseAlgorithm:
    ...
    def __init__(self, agent, hnsw, reward, baseline=None, writer=None, device='cuda'):
        ...
        :type hnsw: lib.hnsw.LearnedHNSW
    ...
    def get_session_batch(self, queries, ground_truth_ids, summarize=True, sample_device=None, **kwargs):
        ...
        # state = nested_map(lambda x: x.to(sample_device) if torch.is_tensor(x) else x, state)
    ...
    def summarize(self, session_records, prefix='train', write_logs=True, **kwargs):
        ...
        num_gt = len(session_records[0]['ground_truth_id'])
        if num_gt > 1:
            recall_all = np.mean([rec['best_vertex_id'] in rec['ground_truth_id']
                                  for rec in session_records])
            counters[prefix + '/recall@%i' % num_gt] = recall_all
            counters[prefix + '/recall@%i_per_distance_computation' % num_gt] = \
                np.mean([int(rec['best_vertex_id'] in rec['ground_truth_id']) / rec['total_distance_computations']
                         for rec in session_records])
    ...
    def evaluate(self, batch_queries, batch_ground_truth_ids, prefix='dev', **kwargs):
        ...
        _ = self.get_session_batch(batch_queries, batch_ground_truth_ids, greedy=True,
                                   summarize=True, prefix=prefix)['summary']
...
class REINFORCE(BaseAlgorithm):
    ...
    def train_on_batch(self, state, from_vertex_ids, to_vertex_ids, actions, rewards, session_index, **kwargs):
        ...
        weights = 1. / session_lengths[session_index]
        ...
        weighted_advantage = weights * (rewards - baseline)
        ...
        from_vertex_ids, to_vertex_ids, actions, weighted_advantage, freqs = \
            self.aggregate_samples(from_vertex_ids, to_vertex_ids, actions, weighted_advantage, device=self.device)
        ...
        J = torch.sum(weighted_advantage * logp_action) / weights.sum()
        ...
        # summarize
        self.writer.add_scalar('train/baseline', float(baseline.mean().item()), global_step=self.step)
        self.writer.add_scalar('train/advantage', float(weighted_advantage.mean().item()), global_step=self.step)
        self.writer.add_scalar('train/entropy', float(ent.item()), global_step=self.step)
        return mean_reward
...
class PPO(REINFORCE):
    ...
    def train_on_batch(self, state, from_vertex_ids, to_vertex_ids, actions,
                       rewards, session_index, aggregate_samples=True, **kwargs):
        session_lengths = torch.bincount(session_index).to(torch.float32)
        weights = 1. / session_lengths[session_index]
        baseline = self.baseline.get(state=state, from_vertex_ids=from_vertex_ids, to_vertex_ids=to_vertex_ids,
                                     rewards=rewards, session_index=session_index, device=self.device, **kwargs)
        ...
        if aggregate_samples:
            ...
            freqs = torch.ones(from_vertex_ids.size(0)).to(device=self.device)
        ...
        for i in range(self.optimizer_steps):
            ...
            objective = torch.min(ratio * weighted_advantage, clipped_ratio * weighted_advantage)
            ...
            if self.early_stop and bool(torch.all(ratio == clipped_ratio).data.cpu().numpy()):
                break
            if self.clip_grad_norm:
                torch.nn.utils.clip_grad_norm_(self.agent.parameters(), self.clip_grad_norm)
            self.opt.step()
...
class TRPO(BaseAlgorithm):
    ...
    def train_on_batch(self, state, from_vertex_ids, to_vertex_ids, actions, rewards, session_index, **kwargs):
        ...
        ratio = torch.exp(logp_action - old_logp_action)  # pi(a|s) / pi_old(a|s)
        loss = -(ratio * weighted_advantage).sum() / weights.sum()
        ...
        self.writer.add_scalar('train/advantage', float(weighted_advantage.mean().item()), global_step=self.step)
        self.writer.add_scalar('train/entropy', float(ent.item()), global_step=self.step)
        self.writer.add_scalar('train/kl', float(kl.item()), global_step=self.step)
        self.writer.add_scalar('train/loss', float(loss.item()), global_step=self.step)
        return mean_reward


class EfficientTRPO(TRPO):
    """ Trust Region Policy Optimization with batch update, see https://arxiv.org/pdf/1502.05477.pdf """

    def __init__(self, agent, hnsw, reward, baseline, samples_in_batch=95000,
                 Fvp_speedup=3, Fvp_type='fim', **kwargs):
        super().__init__(agent, hnsw, reward, baseline, **kwargs)
    ...
    def train_on_batch(self, state, from_vertex_ids, to_vertex_ids, actions, rewards, session_index, **kwargs):
        ...
        batches_advantage = weighted_advantage.split(self.samples_in_batch)
        ...
        for batch_from_vertex_ids, batch_to_vertex_ids, batch_actions, batch_advantage, batch_freqs in \
            ...
            loss = -(ratio * batch_advantage).sum() / weights.sum()
        ...
        # loss_grad /= weights.sum()

        # Forward Fisher vector product
        def Fvp_forward(v):
            # Here we compute Fx to do solve Fx = g using conjugate gradients
            flat_grad_grad_kl = 0
            nbatches = len(batches_freqs) // self.Fvp_speedup
            nbatches = nbatches if nbatches > 0 else len(batches_freqs)
            ...
            flat_grad_grad_kl /= sum([batch_freqs.sum() for batch_freqs in batches_freqs[:nbatches]])  #freqs.sum()
            return flat_grad_grad_kl + v * self.damping
